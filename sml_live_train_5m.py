#!/usr/bin/env python3
"""sml_live_train_5m.py — retrain the SML adapter on 5-MINUTE data.

Same protocol as the 15m trainer but TF=5, reading the freshly downloaded
{SYM}_5min.csv files. Saves to adapter_live_5m. SAVES the adapter.
"""
import json
import math
import os
...
TF = 5
# Broker only serves ~26 days of 5m bars. Chronological split: first ~19d
# train, last ~7d test (locked holdout). Same point-in-time discipline.
...
def main():
    ...
    for s in SYMBOLS:
        r = build_rows(s, TF)
        print(f"{s}: {len(r)} rows", flush=True)
        rows += r
    ...
    aup = sum(1 for r in te if r["label"] == "UP") / max(1, len(te))
    print(f"TEST ACC (sub1500): {tacc:.4f} | always-up {aup:.4f}", flush=True)
    json.dump({"tf": 5, "test_acc": tacc, "test_n": min(1500, len(te)),
               "always_up": aup},
              open(os.path.join(HERE, "live_train_results_5m.json"), "w"))
