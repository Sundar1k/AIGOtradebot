...
def drive(ctx, sim, df, start_idx, window=WINDOW, signal_sink=None):
    ...
    signal_sink: optional selection_validator.SignalSink attached to
    ctx.on_graded_signal (observation-only; records every graded signal and
    matches realized outcomes from sim.trades after the replay)."""
    import bot     # imported here to avoid a cycle (bot imports backtest lazily)
    if signal_sink is not None:
        ctx.on_graded_signal = signal_sink
    trade_state = None
    for i in range(start_idx, len(df)):
        sim.set_bar(i)
        had_pos = sim.pos is not None
        sim.process_exits()                 # close on stop/target, else trail
        if (sim.pos is None and had_pos and sim.trades
                and getattr(ctx, "pattern_skip_signals", None) is not None):
            # process_exits just stopped the position out. Live arms the
            # pattern-lane cooldown in the stop-exit branch of handle_bar; the
            # backtest never reaches that branch (drive nulls trade_state
            # after a broker close), so arm it here from the realized trade to
            # keep backtest = live (2026-08-19: without this, PPO-mode
            # backtests showed zero cooldown effect).
            t = sim.trades[-1]
            if t.r <= -1.0 and t.strategy == "pattern":
        ...
        if sim.pos is not None and sim.pos.get("strategy") is None:
            # Tag the open position with the strategy that opened it. In PPO
            # mode handle_bar returns trade_state (carries .strategy); in
            # fixed-RR mode it returns None but ctx.last_trade holds the
            # bookkeeping — use whichever is populated (2026-08-19: without
            # this, fixed-RR backtests tagged every trade "?").
            src = trade_state or getattr(ctx, "last_trade", None)
...
def run_backtest(symbol="NQ", start=None, end=None, log_signals=None):
    import bot     # imported here to avoid a cycle (bot imports backtest lazily)
