...
def manage_trail(tee, policy, client, account_id, contract_id, tick_size,
    ...
    # BREAKEVEN RATCHET (2026-08-23): once the trade shows BE_TRIGGER R, the
    # stop must never sit worse than entry again. Validated on 281 ledger
    # trades: converts −1R reversals into scratches (WR 64%→64% counted as
    # wins-at-breakeven excluded, but avgR +1.33→+1.47 and Aug avgR −0.04→+0.25).
    be_trigger = float(os.environ.get("AUTOTRADE_BE_TRIGGER", "0.75"))
    if be_trigger > 0 and st["peak_R"] >= be_trigger:
        be_price = st["entry"] + sign * 2 * tick_size   # entry + 2 ticks (costs)
        if (st["stop"] < be_price) if sign > 0 else (st["stop"] > be_price):
            try:
                order = client.working_stop_order(account_id, contract_id)
                if order is not None:
                    client.modify_stop_price(account_id, order["id"], _snap(be_price, sign, tick_size))
                    log.info("   %s  🔒 breakeven ratchet (%.2fR seen) → stop %.2f",
                             stamp, st["peak_R"], be_price)
            except Exception as e:
                log.warning("   %s  breakeven modify rejected (%s) — retry next bar",
                            stamp, e)
            st["stop"] = max(st["stop"], be_price) if sign > 0 else min(st["stop"], be_price)
            ...
            if abs(new_stop - st["stop"]) >= tick_size:
                try:
                    ...
                    log.warning("   %s  stop modify rejected (%s) — holding %.2f "
                                "(attempted %.2f, min-dist likely)",
                                stamp, e, st["stop"], new_stop)
            else:
