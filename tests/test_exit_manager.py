...
def test_giveback_wick_cross_closes_at_market():
    ...
    # 2026-08-23 BE ratchet: peak(≥0.75R seen) locks stop to breakeven BEFORE
    # the wick-cross market close — one BE modify is expected.
    assert len(client.modified) == 1 and abs(client.modified[0] - 99.5) < TICK
...
def test_no_cross_reprices_stop():
    ...
    # 2026-08-23: BE ratchet fires once (stop→99.5) then the trail tightens —
    # two modifies total, both below the original 110.
    assert len(client.modified) == 2
    assert all(m < 110.0 for m in client.modified)
